...
#define MAP_CUTS_MAX_COMPUTE 2000
// the largest number of cuts used

#define MAP_CUTS_MAX_USE 250

#define MAP_CUTS_MAX_USE_Ml 12
...
static int s_HashPrimes[10] = {109, 499, 557, 619, 631,
                               709, 797, 881, 907, 991};

static Map_Cut_t *Map_CutCompute(Map_Man_t *p, Map_CutTable_t *pTable,
                                 Map_Node_t *pNode);
static void Map_CutFilter(Map_Man_t *p, Map_Node_t *pNode);
static Map_Cut_t *Map_CutMergeLists(Map_Man_t *p, Map_CutTable_t *pTable,
                                    Map_Cut_t *pList1, Map_Cut_t *pList2,
                                    int fComp1, int fComp2);
static int Map_CutMergeTwo(Map_Cut_t *pCut1, Map_Cut_t *pCut2,
                           Map_Node_t *ppNodes[], int nNodesMax);
static Map_Cut_t *Map_CutUnionLists(Map_Cut_t *pList1, Map_Cut_t *pList2);
static int Map_CutBelongsToList(Map_Cut_t *pList, Map_Node_t *ppNodes[],
                                int nNodes);
...
static int Map_CutTableLookup(Map_CutTable_t *p, Map_Node_t *ppNodes[],
                              int nNodes);
static Map_Cut_t *Map_CutTableConsider(Map_Man_t *pMan, Map_CutTable_t *p,
                                       Map_Node_t *ppNodes[], int nNodes);
static void Map_CutTableRestart(Map_CutTable_t *p);

static Map_Cut_t *Map_CutSortCuts(Map_Man_t *pMan, Map_CutTable_t *p,
                                  Map_Cut_t *pList);
...
static unsigned Map_CutComputeTruth(Map_Man_t *p, Map_Cut_t *pCut,
                                    Map_Cut_t *pTemp0, Map_Cut_t *pTemp1,
                                    int fComp0, int fComp1);
...
#define Map_ListForEachCutSafe(pList, pCut, pCut2)                             \
  for (pCut = pList, pCut2 = pCut ? pCut->pNext : NULL; pCut;                  \
       pCut = pCut2, pCut2 = pCut ? pCut->pNext : NULL)
    ...
    for (pNode = pMan->pBins[i]; pNode; pNode = pNode->pNext)
      for (pCut = pNode->pCuts; pCut; pCut = pCut->pNext)
        if (pCut->nLeaves > 1) // skip the elementary cuts
        {
          nCuts++;
          /*
                              if ( Map_CutRegular(pCut->pOne)->nLeaves == 5 &&
             Map_CutRegular(pCut->pTwo)->nLeaves == 5 ) nCuts55++; if (
             Map_CutRegular(pCut->pOne)->nLeaves == 5 ||
             Map_CutRegular(pCut->pTwo)->nLeaves == 5 ) nCuts5x++; else if (
             Map_CutRegular(pCut->pOne)->nLeaves == 4 ||
             Map_CutRegular(pCut->pTwo)->nLeaves == 4 ) nCuts4x++; else if (
             Map_CutRegular(pCut->pOne)->nLeaves == 3 ||
             Map_CutRegular(pCut->pTwo)->nLeaves == 3 ) nCuts3x++;
          */
          //                    pCounts[ Map_CutRegular(pCut->pOne)->nLeaves
          //                    ]++; pCounts[
          //                    Map_CutRegular(pCut->pTwo)->nLeaves ]++;
        }
  //    printf( "Total cuts = %6d. 55 = %6d. 5x = %6d. 4x = %6d. 3x = %6d.\n",
  //    nCuts, nCuts55, nCuts5x, nCuts4x, nCuts3x );

  //    printf( "Total cuts = %6d. 6= %6d. 5= %6d. 4= %6d. 3= %6d. 2= %6d. 1=
  //    %6d.\n",
  //        nCuts, pCounts[6], pCounts[5], pCounts[4], pCounts[3], pCounts[2],
  //        pCounts[1] );
  return nCuts;
}
...
***********************************************************************/
Map_Cut_t *Map_CutCompute(Map_Man_t *p, Map_CutTable_t *pTable,
                          Map_Node_t *pNode) {
  Map_Node_t *pTemp;
  ...
  // merge the lists
  pList =
      Map_CutMergeLists(p, pTable, pList1, pList2, Map_IsComplement(pNode->p1),
                        Map_IsComplement(pNode->p2));
  ...
  if (pNode->pRepr && Map_NodeComparePhase(pNode, pNode->pRepr)) {
    Map_ListForEachCut(pNode->pCuts, pCut) pCut->Phase = 1;
  }
  /*
      {
          int i, Counter = 0;;
...
/*Free the whole list */
void free_List(Map_Cut_t *node, Map_Man_t *p) {
  Map_Cut_t *cur, *next;
...
void Map_CutFilter_ML(Map_Man_t *p) {
  Map_Node_t *pNode;
  int i;
  int limit = ML_CutLimitForMode( g_mode );
  int minKeep = ML_CutMinKeepForMode( g_mode );
  float scoreWindow = ML_CutScoreWindowForMode( g_mode );

  for (i = 0; i < p->vMapObjs->nSize; i++) {
    pNode = p->vMapObjs->pArray[i];
    Map_Cut_t *pTemp;
    Map_Cut_t *pickedHead, *pickedTail, *restHead;
    Map_Cut_t *cuts[MAP_CUTS_MAX_COMPUTE];
    float scores[MAP_CUTS_MAX_COMPUTE];
    float bestScore;
    int count, keepCount, selectedCount, j, k, best;

    if (!Map_NodeIsAnd(pNode) || pNode->pCuts == NULL)
      continue;

    count = 0;
    for (pTemp = pNode->pCuts->pNext; pTemp; pTemp = pTemp->pNext) {
      if (count == MAP_CUTS_MAX_COMPUTE)
        break;
      cuts[count] = pTemp;
      scores[count] = ML_ScoreCut( pTemp, pNode );
      count++;
    }
    ...
    keepCount = count < limit ? count : limit;
    for (j = 0; j < keepCount; j++) {
      ...
      for (k = j + 1; k < count; k++) {
        if (scores[k] > scores[best])
          ...
          if (cuts[k]->nLeaves < cuts[best]->nLeaves)
            best = k;
          else if (cuts[k]->nLeaves == cuts[best]->nLeaves &&
                   cuts[k]->nVolume < cuts[best]->nVolume)
            best = k;
        }
      }
      if (best != j) {
        Map_Cut_t *tmpCut = cuts[j];
    ...
    if (selectedCount > minKeep) {
      bestScore = scores[0];
      selectedCount = minKeep;
      while (selectedCount < keepCount &&
             scores[selectedCount] >= bestScore - scoreWindow)
        selectedCount++;
    }
    ...
    for (j = 0; j < selectedCount; j++) {
      cuts[j]->pNext = NULL;
      if (pickedHead == NULL)
        pickedHead = pickedTail = cuts[j];
      else {
        pickedTail->pNext = cuts[j];
        pickedTail = cuts[j];
      }
    }

    restHead = NULL;
    for (j = selectedCount; j < count; j++) {
      cuts[j]->pNext = restHead;
      restHead = cuts[j];
    }

    pNode->pCuts->pNext = pickedHead;
    if (restHead)
      free_List(restHead, p);
...
void PrintCutStats(Map_Man_t *p) {
  float qor = p->AreaFinal > 0.0 ? p->AreaFinal : p->AreaBase;
  int candidateCuts = Map_MappingCountAllCuts(p);
  int cutsUsed = Map_MappingCountUsedCuts(p);
  float cpu_time = (float)p->timeTotal;
  float post_ml_time = (float)p->time1;
  printf("RESULT g_mode=%d qor=%.2f cuts_used=%d cpu_time=%.2f post_ml_time=%.2f\n", g_mode, qor, cutsUsed, cpu_time, post_ml_time);
  printf("CUT_STATS g_mode=%d candidate_cuts=%d\n", g_mode, candidateCuts);
}
...
Map_Cut_t *Map_CutMergeLists(Map_Man_t *p, Map_CutTable_t *pTable,
                             Map_Cut_t *pList1, Map_Cut_t *pList2, int fComp1,
                             int fComp2) {
  ...
  for (i = 0; i < nCuts1; i++) {
    for (k = 0; k <= i; k++) {
      ...
      //            if ( p->nVarsMax == 5 )
      //            pCut->uTruth = Map_CutComputeTruth( p, pCut, pTemp1, pTemp2,
      //            fComp1, fComp2 );
...
***********************************************************************/
int Map_CutMergeTwo(Map_Cut_t *pCut1, Map_Cut_t *pCut2, Map_Node_t *ppNodes[],
                    int nNodesMax) {
  ...
  for (i = 0; i < nTotal - 1; i++) {
    ...
    for (k = i + 1; k < nTotal; k++)
      //            if ( ppNodes[k] < ppNodes[min] ) // reported bug fix
      //            (non-determinism!)
...
Map_Cut_t *Map_CutUnionLists(Map_Cut_t *pList1, Map_Cut_t *pList2) {
  ...
  pRoot = pList1;
  Map_ListForEachCut(pList1, pTemp) pRoot = pTemp;
...
void Map_CutListPrint(Map_Man_t *pMan, Map_Node_t *pRoot) {
  Map_Cut_t *pTemp;
  int Counter;
  for (Counter = 0, pTemp = pRoot->pCuts; pTemp;
       pTemp = pTemp->pNext, Counter++) {
    printf("%2d : ", Counter + 1);
...
***********************************************************************/
Map_Cut_t *Map_CutTableConsider(Map_Man_t *pMan, Map_CutTable_t *p,
                                Map_Node_t *ppNodes[], int nNodes) {
  Map_Cut_t *pCut;
  int Place, i;
  //    abctime clk;
...
***********************************************************************/
Map_Cut_t *Map_CutSortCuts(Map_Man_t *pMan, Map_CutTable_t *p,
                           Map_Cut_t *pList) {
  Map_Cut_t *pListNew;
  int nCuts, i, nCutsMaxUse;
  //    abctime clk;
  ...
  nCutsMaxUse = ML_CutPrefilterLimitForMode( g_mode );
  if (nCuts > nCutsMaxUse - 1) {
    // free the remaining cuts
    for (i = nCutsMaxUse - 1; i < nCuts; i++)
...
***********************************************************************/
unsigned Map_CutComputeTruth(Map_Man_t *p, Map_Cut_t *pCut, Map_Cut_t *pTemp0,
                             Map_Cut_t *pTemp1, int fComp0, int fComp1) {
